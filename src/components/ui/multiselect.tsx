...
const multiSelectVariants = cva("m-1 transition-all duration-300 ease-in-out", {
  variants: {
    variant: {
      default:
        "bg-black text-white border-black hover:bg-shark-gray-950 rounded-[6px]",
      secondary:
      ...
      inverted: "inverted",
      transparent:
        "bg-transparent border-transparent text-current hover:bg-transparent rounded-[6px]",
      nido: "bg-lightning-yellow-600/90 text-white border-transparent rounded-[6px]",
    },
    badgeAnimation: {
...
interface MultiSelectOption {
  ...
  value: string;
  /** Optional label override for content that needs to be visible but not searchable. */
  displayValue?: string | React.ReactNode;
  /** Optional icon component to display alongside the option. */
  icon?: React.ComponentType<{ className?: string }>;
  /** Whether this option is disabled */
  disabled?: boolean;
  /** Additional data that can be used in custom renderers */
  data?: unknown;
...
/**
 * Custom action configuration for the MultiSelect component
 */
export interface CustomAction {
  /** Button component that will appear below the options list */
  actionButton: React.ReactNode;

  /**
   * Content to display when the action button is clicked
   * Accepts a function that receives an onClose callback
   */
  content:
    | React.ReactNode
...
interface MultiSelectGroup {
  ...
  /** Custom renderer for option labels in this group */
  renderLabel?: (option: MultiSelectOption) => React.ReactNode;
  /** Custom renderer for selected content in this group */
  renderSelectedContent?: (option: MultiSelectOption) => React.ReactNode;
}
...
interface MultiSelectProps
  ...
  /**
   * Custom label renderer for dropdown options.
   * Receives option and returns ReactNode for custom label rendering.
   * Useful for displaying additional information like dates, icons, etc.
   * Optional, defaults to option.label.
   */
  renderLabel?: (option: MultiSelectOption) => React.ReactNode;

  /**
   * Custom content renderer for selected badges.
   * Receives option and returns ReactNode for custom badge content.
   * Useful for compact display of selected items.
   * Optional, defaults to option.label.
   */
  renderSelectedContent?: (option: MultiSelectOption) => React.ReactNode;

  /**
   * Custom action configuration object containing the action button and content
   */
  customAction?: CustomAction;

  /**
   * Controlled search input value for async search
   */
  searchValue?: string;

  /**
   * Callback for when the search input value changes for async search
   */
  onSearchChange?: (value: string) => void;

  /**
   * A slot to render custom content at the bottom of the list, e.g., a loading spinner for infinite scroll.
   */
...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
  (
    ...
    const [isShowingCustomContent, setIsShowingCustomContent] =
    ...
    }, [defaultValue, onValueChange, handleSearchChange]);
    ...
    // Helper function to get group renderer for an option
    const getGroupRenderer = React.useCallback(
      (option: MultiSelectOption): MultiSelectGroup | null => {
        if (!isGroupedOptions(options)) return null;
    ...
    // Sync external value prop with internal state (controlled component behavior)
    React.useEffect(() => {
    ...
    }, [isPopoverOpen, handleSearchChange]);
        ...
        >
          ...
          <PopoverTrigger asChild>
            <Button
              ...
              className={cn(
                "flex p-1 rounded-md border min-h-12 h-auto items-center justify-between bg-inherit hover:bg-gray-50 [&_svg]:pointer-events-auto text-shark-gray-500 active:bg-gray-100",
                        ...
                        return (
                          <Badge
                            ...
                            className={cn(
                              ...
                              responsiveSettings.compactMode && "px-1.5 py-0.5",
                              screenSize === "mobile" &&
                          ...
                          >
                            ...
                            >
                              {(() => {
                                const group = getGroupRenderer(option);
                                if (group?.renderSelectedContent) {
                                  return group.renderSelectedContent(option);
                                }
                                if (renderSelectedContent) {
                                  return renderSelectedContent(option);
                                }
                                return option.label;
                              })()}
                            </span>
                            <div
                              ...
                              className="ml-2 h-4 w-4 cursor-pointer hover:bg-white/20 rounded-sm focus:outline-none focus:ring-1 focus:ring-white/50 flex items-center justify-center"
                            >
                              <XCircle
                                className={cn(
                                  "h-3.5! w-3.5!",
          ...
          <PopoverContent
            ...
            className={cn(
              ...
              screenSize === "tablet" && "w-[511px]",
              screenSize === "desktop" && "min-w-[300px]",
              "overflow-hidden max-h-[10rem] flex flex-col",
              popoverClassName
            )}
            style={{
          ...
          >
            {!isShowingCustomContent ? (
              <Command
                filter={onExternalSearchChange ? () => 1 : filter}
                ...
                {!isLoading ? (
                  <>
                    <CommandList className="flex-1 overflow-y-auto">
                      ...
                      {!hideSelectAll && !currentSearchValue && (
                        <CommandGroup>
                          <CommandItem
                            key="all"
                            onSelect={toggleAll}
                            ...
                            <div
                              className={cn(
                                ...
                                selectedValues.length ===
                                  getAllOptions().filter((opt) => !opt.disabled)
                                    .length
                                  ? "bg-blue-violet-900 border-blue-violet-800 text-white"
                    ...
                    <div className="border-t bg-background">
                      {customAction?.actionButton && (
                        <>
                          ...
                          <CommandGroup>
                            <div
                              onClick={() => setIsShowingCustomContent(true)}
                              className="cursor-pointer"
                            >
                              {customAction.actionButton}
                            </div>
              ...
              <div className="p-1">
                <div className="flex justify-between items-center p-2">
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => setIsShowingCustomContent(false)}
                    className="flex items-center gap-1"
                  >
                    <ArrowLeft className="h-4 w-4 text-blue-violet-500" />
                    <span className="text-blue-violet-500">Atrás</span>
                  </Button>
                </div>
                <div className="p-2 mb-2">
                  {typeof customAction?.content === "function"
                    ? customAction.content({
                        onClose: () => setIsShowingCustomContent(false),
                      })
                    : customAction?.content}
                </div>
              </div>
